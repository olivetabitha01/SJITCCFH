...
def update_sensor():
    ...
    # Update sensor data
    for key in ['Temperature', 'Humidity', 'CO2_Emitted_Cars', 'Sunlight']:
        if key in incoming_data:
            sensor_data[key] = incoming_data[key]
            # Update sensor status
            sensor_key = key.lower().replace('_', '_')
...
def get_sensor():
    ...
    if sensor_data['last_updated']:
        ...
        for sensor_key in ['temperature', 'humidity', 'co2', 'light']:
            if age_seconds > 30:
